...
// Canonical definition, and the single source of truth for both callers:
// the serverless relay (api/healthsync/*) imports it relatively, and the
// PWA re-exports it from src/services/healthSyncSchema.ts.
//
// It lives under api/ rather than src/ because Vercel bundles ONLY the
// api directory into each serverless function — a handler importing from
// src/ compiles fine but throws ERR_MODULE_NOT_FOUND at runtime.
//
// Keep this file dependency-free and free of browser- or db-specific
// imports: no npm packages, no node: builtins, no DOM globals. It must
// run unchanged in the Node function and in the browser bundle.
//
// Payload shape only. Sleep scoring and the Dexie upsert are deliberately
// NOT here — those stay client-side in src/services/healthSync.ts.
